...
const UserList = () => {
  ...
  const getStatusChip = (isActive, isStaff) => {
    if (isStaff) {
      return <Chip label="管理员" color="primary" icon={<AdminIcon />} size="small" />;
    }
    return isActive ? (
      <Chip label="正常" color="success" icon={<CheckCircleIcon />} size="small" />
    ) : (
              ...
              <TableBody>
                {users.map((user) => (
                  <TableRow key={user.id} hover>
                    ...
                    <TableCell>{getStatusChip(user.is_active, user.is_staff)}</TableCell>
